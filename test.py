...
print("TRAINING DONE!")
np.savez('model69.npz', layers.get_all_param_values(NET))

with np.load('model.npz') as f:
     param_values = [f['arr_%d' % i] for i in range(len(f.files))]
x = layers.set_all_param_values(NET, param_values[0])
print(param_values)
...
# all_params = nntools.layers.get_all_paramns(NET)
# all_param_values =[p.get_value(())]
#NET.save_weights_to("memeModel")
#layers.save_weights_to("memeModel")
#np.savez('model.npz', *lasagne.layers.get_all_param_values())